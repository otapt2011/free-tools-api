/**
 * background-app.js – UI integration with fixed canvas.
 * Padding adds margin inside the canvas without shrinking the image.
 */
(function() {
  'use strict';
  ...
  console.log('[sShot-app] Initializing UI (fixed canvas, padding-inset mode).');
  ...
  async function handleFile(file) {
    ...
    reader.onload = async (e) => {
      try {
        ...
        // Compute natural size (with current padding)
  ...
  // ----- NEW RENDER: padding adds margin, image scales to fit inner area -----
  function renders() {
    if (!maker || !hasImage || fixedWidth === 0 || fixedHeight === 0) {
      console.warn('[render] Skipped: missing data');
      return;
    }
    ...
    const frameScale = state.frameScale;
    const frameColor = state.frameColor;
    const imageOffsetX = state.imageOffsetX;
    const imageOffsetY = state.imageOffsetY;
    const frameOffsetX = state.frameOffsetX;
    const frameOffsetY = state.frameOffsetY;

    const image = maker.state.image;
    if (!image) return;

    // Compute the inner rectangle (image area) with margins
    const innerX = padding;
    const innerY = padding;
    const innerW = fixedWidth - 2 * padding;
    const innerH = fixedHeight - 2 * padding;
    if (innerW <= 0 || innerH <= 0) {
      console.warn('[render] Padding too large, inner area zero.');
      return;
    }
    ...
    // 1. Draw gradient background (full canvas)
    const deg = parseFloat(gradientAngle) || 135;
    const rad = deg * Math.PI / 180;
    const cx = fixedWidth/2, cy = fixedHeight/2;
    const r = Math.sqrt(fixedWidth*fixedWidth + fixedHeight*fixedHeight) / 2;
    ...
    // 2. Draw frame (if enabled) on the entire canvas
    if (frameEnabled && frameType !== 'none') {
      const renderer = maker.renderer;
      ctx.save();
      ctx.translate(frameOffsetX, frameOffsetY);
      renderer.drawFrame(ctx, frameType, 0, 0, fixedWidth, fixedHeight, frameScale, frameColor);
      ctx.restore();
    }

    // 3. Draw the image within the inner rectangle (with padding margins)
    const imgW = image.width;
    const imgH = image.height;

    // Determine target aspect ratio from UI
    ...
    // Fit cropped image into innerW x innerH
    const drawW = cropW;
    const drawH = cropH;
    ...
    // Draw the image with clipping for corner radius
    ctx.save();
    if (cornerRadius > 0) {
      const radius = Math.min(cornerRadius, finalW/2, finalH/2);
      ctx.beginPath();
      ctx.moveTo(dx + radius, dy);
      ctx.quadraticCurveTo(dx + finalW, dy, dx + finalW, dy + radius);
      ctx.quadraticCurveTo(dx + finalW, dy + finalH, dx + finalW - radius, dy + finalH);
      ctx.quadraticCurveTo(dx, dy + finalH, dx, dy + finalH - radius);
  ...
  // Inner rectangle (image area) with margins
  const innerX = padding;
  const innerY = padding;
  const innerW = fixedWidth - 2 * padding;
  const innerH = fixedHeight - 2 * padding;
  ...
  // 1. Background gradient (full canvas, no rounding)
  const deg = parseFloat(gradientAngle) || 135;
  const rad = deg * Math.PI / 180;
  const cx = fixedWidth/2, cy = fixedHeight/2;
  const r = Math.sqrt(fixedWidth*fixedWidth + fixedHeight*fixedHeight) / 2;
  ...
  // 2. Frame (if enabled) – full canvas, no rounding
  if (frameEnabled && frameType !== 'none') {
    const renderer = maker.renderer;
    ctx.save();
    ctx.translate(frameOffsetX, frameOffsetY);
  ...
  // 3. Image with corner radius on the image rectangle ONLY
  const imgW = image.width;
  const imgH = image.height;
  ...
  // Fit cropped image into inner rectangle
  const drawW = cropW;
  const drawH = cropH;
  const scaleX = innerW / drawW;
  const scaleY = innerH / drawH;
  ...
  // Apply corner radius to the image rectangle
  ctx.save();
  if (cornerRadius > 0) {
    const radius = Math.min(cornerRadius, finalW/2, finalH/2);
    ctx.beginPath();
  ...
  function init() {
    if (!el.canvas) return;
    // Create maker with fixedSize: true to prevent canvas resizing
    maker = new BackgroundMaker(el.canvas, { fixedSize: true });
    enableControls(false);
    enableButtons(false);
    setupFileUpload();
    setupControls();
